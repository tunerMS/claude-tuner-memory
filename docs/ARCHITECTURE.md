...
### Recall side (hooks/)
`session-context.sh` (SessionStart hook) closes the loop: it maps the session's cwd to `sessions/<project>.md` and injects the 🔖 current-state block automatically — worktree-aware (the repo name comes from `git-common-dir`, not the worktree dir) with a longest-prefix fallback for suffixed checkout names. No exact file → it prints a compact index of available projects instead of guessing (the summarizer occasionally confuses similar slugs, so a visible index beats clever mapping). `testrun-capture.sh` (PostToolUse hook on Bash) greps gradle/pytest summaries out of command output into `~/.claude/memory-testruns/<repo>.log` (last 30 runs), which session-context surfaces alongside the state block. Both hooks skip background `claude` calls: harvester sets `CC_BACKGROUND=1` on its own CLI invocations and the analyzer exports `HOMUNCULUS_ANALYZING=1`.

## homunculus
`capture-guard.sh` + `_capture.py` safely parse hook JSON from stdin (no code injection) and append observations, honoring the `exclude_paths` denylist. `analyze.sh` (launchd, every 30m) distills observations into instincts via `claude --print` only when enough have accumulated, then archives them. The prompt includes the ids+triggers of already-known instincts: a pattern that semantically matches one comes back as an id-only confidence bump instead of a new near-duplicate slug. `_write_instincts.py` writes/merges instinct files (confidence grows on repeat); an id-only update bumps `confidence`/`last_observed` in place without rewriting the body (hand-merged triggers/actions survive), and an unknown id without content is skipped rather than written as an empty stub. `recall-instincts.sh` (SessionStart hook) injects instincts above a confidence threshold back into the session, capped at the top-25 by confidence to keep the injection compact.
