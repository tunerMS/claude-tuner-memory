...
## homunculus
`capture-guard.sh` + `_capture.py` safely parse hook JSON from stdin (no code injection) and append observations, honoring the `exclude_paths` denylist. `analyze.sh` (launchd, every 5m) distills observations into instincts via `claude --print` only when enough have accumulated, then archives them. `_write_instincts.py` writes/merges instinct files (confidence grows on repeat). `recall-instincts.sh` (SessionStart hook) injects instincts above a confidence threshold back into the session.
